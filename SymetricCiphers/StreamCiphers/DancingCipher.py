...
bytes_enc_msg = bytes.fromhex(msg_enc)
state = xor(msg[0:64], bytes_enc_msg[0:64])
tab_state = [int.from_bytes(state[4*i:4*i+4], 'little') for i in range(16)]

for i in range(10):
    tab_state = inner_block(tab_state)

flag_enc = 'b6327e9a2253034096344ad5694a2040b114753e24ea9c1af17c10263281fb0fe622b32732'
bytes_flag_enc = bytes.fromhex(flag_enc)

iv2 = bytes.fromhex('a99f9a7d097daabd2aa2a235')

key = words_to_bytes(tab_state[4:12])
print('key: ', key)

c = ChaCha20()
flag = c.decrypt(bytes_flag_enc, key, iv2)
